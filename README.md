...
- **Objects** — `type Point:` defines a class with **fields** (`make x`, `make y = 0` for a default) and **methods** (`task length(self): …`, where `self` is the object). Build one with `Point(3, 4)`, read/write fields with `p.x` and `set p.x = …`, call methods with `p.length()`, and `kind_of(p)` is `"Point"`. Different types sharing a method name dispatch to the right one at run time (polymorphism). **Inheritance** too — `type Dog from Animal:` inherits the parent's fields and methods, with **overriding** (virtual dispatch) and `is_a(d, "Animal")` checks
